...
import { Size, Point, area, isIn, getCoords, getIdx } from './modules/rect'
import { numRange, mapIterable } from './modules/range_iterable'
import update from 'immutability-helper'
import './index.css';

class Board {
  constructor(size: Size, squares: Array<string | null> = new Array(area(size)).fill(null)) {
    this.size = size;
    this.squares = squares;
  }
  resize(newSize: Size): Board {
    let newSquares = new Array(area(newSize)).fill(null);
    for (let idx = 0; idx < area(newSize); ++idx) {
      const y = Math.floor(idx / newSize.width);
      const x = idx % newSize.width;
...
function matchWinConditionAtOrigin(board: Board, winCondition: Board, topLeft: Point): string | null {
  const boardRect = { origin: { x: 0, y: 0 }, size: board.size };
  const winConditionRect = { origin: topLeft, size: winCondition.size };
  if (!isIn(winConditionRect, boardRect)) {
    return null;
  }
  let potentialWinner: string | null = null;
  for (let idx = 0; idx < area(winCondition.size); ++idx) {
    const coords = getCoords(winConditionRect, idx);
    const boardIdx = getIdx(boardRect, coords);
    const winEntry = winCondition.squares[idx];
    const boardEntry = board.squares[boardIdx];
    if (winEntry && !boardEntry) {
      return null;
    }
    // update and check potential winner
    potentialWinner = potentialWinner || (winEntry && boardEntry);
    if (winEntry && (boardEntry !== potentialWinner)) {
      return null;
    }
  }
...
function matchWinCondition(board: Board, winCondition: Board): VictoryData | null {
  const region = { origin: { x: 0, y: 0 }, size: { width: board.size.width - winCondition.size.width + 1, height: board.size.height - winCondition.size.height + 1 } }
  for (let idx = 0; idx < area(region.size); ++idx) {
    const origin = getCoords(region, idx);
